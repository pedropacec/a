...
// Uso: node render.mjs [feed|story|still] [dir-de-frames]
//   feed  -> out/divine-therapy-feed.mp4  (1080x1350)
//   story -> out/divine-therapy-story.mp4 (1080x1920)
//   still -> out/preview-still.png (frame parado, para conferir a arte)
...
const url = `file://${here}/flyer.html?render=1${story ? '&format=story' : ''}`;
...
if (mode === 'still') {
  ...
  await page.screenshot({ path: `${here}/out/preview-still.png` });
  console.log('still -> out/preview-still.png');
} else {
  ...
  const outFile = `${here}/out/divine-therapy-${mode}.mp4`;
  execSync(
